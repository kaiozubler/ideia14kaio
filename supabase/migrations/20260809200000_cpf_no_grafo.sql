-- Inclui o CPF de cada paciente no nó retornado por grafo_familiar,
-- usado no painel expandido ao clicar em uma pessoa no mapa familiar.
...
AS $$
  ...
  nos AS (
    SELECT p.paciente_id, p.name, p.sexo, p.data_nascimento, p.cpf, coalesce(p.cids,'[]'::jsonb) AS cids
  ...
  SELECT jsonb_build_object(
    'nodes', coalesce((SELECT jsonb_agg(jsonb_build_object('id',paciente_id,'nome',name,'sexo',sexo,'nascimento',data_nascimento,'cpf',cpf,'cids',cids)) FROM nos),'[]'::jsonb),
